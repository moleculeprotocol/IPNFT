...
import { ITokenVesting, ROLE_CREATE_SCHEDULE } from "../ITokenVesting.sol";

struct StakingInfo {
    //e.g. VITA DAO token
    IERC20Metadata stakedToken;
    ITokenVesting stakesVestingContract;
...
contract StakedLockingCrowdSale is LockingCrowdSale {
    ...
    event UpdatedTrustedITokenVestings(ITokenVesting indexed tokenVesting, bool trusted);
     ...
     * @param stakesVestingContract the ITokenVesting contract to trust
     */
    function trustVestingContract(ITokenVesting stakesVestingContract) external onlyOwner {
        if (!stakesVestingContract.hasRole(ROLE_CREATE_SCHEDULE, address(this))) {
            revert UnmanageableVestingContract();
        }
        trustedVestingContracts[address(stakesVestingContract)] = true;
        emit UpdatedTrustedITokenVestings(stakesVestingContract, true);
    }

    function untrustVestingContract(ITokenVesting stakesVestingContract) external onlyOwner {
        trustedVestingContracts[address(stakesVestingContract)] = false;
        emit UpdatedTrustedITokenVestings(stakesVestingContract, false);
    }
     ...
     * @param stakesVestingContract the ITokenVesting contract for vested staking tokens. Will revert when not trusted.
     ...
     *        NOTE: If `lockingDuration` is < 7 days, the the vesting contract schedules will stil have a 7 days cliff as required by the underlying ITokenVesting contract.
    ...
    function claim(uint256 saleId, uint256 tokenAmount, uint256 refunds) internal virtual override {
        ...
        //the minimum vesting duration of `ITokenVesting` is 7 days
