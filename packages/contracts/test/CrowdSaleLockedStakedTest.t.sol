...
contract CrowdSaleLockedStakedTest is Test {
    ...
    function testStakeLockingCrowdSalesBadParameters() public {
        ...
        crowdSale.startSale(_sale, IERC20Metadata(address(0)), ITokenVesting(address(0)), 0, 60 days);

        vm.expectRevert(); //need to bring a stake vesting contract
        crowdSale.startSale(_sale, daoToken, ITokenVesting(address(0)), 0, 60 days);
    ...
    function testOverbiddingAndRefunds() public {
        ...
        */
        ...
        assertEq(auctionITokenVesting.balanceOf(bidder), 300_000 ether);
        ...
        assertEq(auctionITokenVesting.balanceOf(bidder2), 100_000 ether);
    ...
    function testUnevenOverbiddingAndPriceAndRefunds() public {
        ...
        TimelockedToken auctionITokenVesting = crowdSale.lockingContracts(address(auctionToken));
        assertEq(auctionITokenVesting.balanceOf(bidder), 230188679245283018800000);
        assertEq(auctionITokenVesting.balanceOf(bidder2), 169811320754716980800000);
    ...
    function testUnsuccessfulSaleClaims() public {
        ...
        assertEq(auctionITokenVesting.balanceOf(bidder), 0);
    }
