...
contract IPNFT is ERC1155, Ownable, Pausable, ERC1155Burnable, ERC1155Supply, ERC1155URIStorage {
    ...
    event ReservationURIUpdated(string tokenURI, address indexed reserver, uint256 indexed reservationId);
    event TokenMinted(string tokenURI, address indexed owner, uint256 indexed tokenId, uint256 shares);
    ...
    function mintReservation(address to, uint256 reservationId) public payable returns (uint256 tokenId) {
        return mintReservation(to, reservationId, reservations[reservationId].tokenURI);
    }

    function mintReservation(address to, uint256 reservationId, string memory tokenURI)
        public
        payable
        returns (uint256 tokenId)
    {
        require(msg.value >= mintPrice, "Ether amount sent is too small");
        require(reservations[reservationId].reserver == _msgSender(), "IP-NFT: caller is not reserver");
    ...
    function updateReservationURI(uint256 reservationId, string calldata _tokenURI) external {
        require(
            reservations[reservationId].reserver == _msgSender(), "IP-NFT: Reservation not valid or not owned by you"
        );
