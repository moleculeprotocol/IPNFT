...
struct StakingConfig {
    ...
    TokenVesting stakingVestingContract;
    uint256 auctionInBiddingPrice;
    uint256 stakes; //initialize with 0
}
...
contract StakedVestedCrowdSale is VestedCrowdSale {
    ...
    function startSale(Sale memory sale, IERC20 stakedToken, TokenVesting stakeVesting, uint256 initialPrice, uint256 cliff, uint256 duration)
        public
        returns (uint256 saleId)
    {
        return startSale(sale, StakingConfig(stakedToken, stakeVesting, initialPrice, 0), cliff, duration);
    }

    function settle(uint256 saleId) public override {
        super.settle(saleId);
        StakingConfig memory staking = salesStaking[saleId];
        staking.stakedToken.approve(address(staking.stakingVestingContract), staking.stakes);
    }
    ...
    function placeBid(uint256 saleId, uint256 biddingTokenAmount) public override {
        ...
        staking.stakes += stakedTokenAmount;
    ...
    function claim(uint256 saleId) public override returns (uint256 auctionTokens, uint256 refunds) {
        ...
        TokenVesting vestingContract = salesStaking[saleId].stakingVestingContract;
        VestingConfig memory vestingConfig = salesVesting[saleId];

        vestingContract.createPublicVestingSchedule(msg.sender, block.timestamp, vestingConfig.cliff, vestingConfig.duration, 60, _contribution);
    }
}
